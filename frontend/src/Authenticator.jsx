...
const Authentication = () => {
    ...
    const fetchSchemas = async () => {
        try {
            setLoading(true);
            const response = await fetch('https://apivengers.onrender.com/api/schemas');
    ...
    const fetchEndpointsForSchema = async (schemaName) => {
        try {
            const response = await fetch(`https://apivengers.onrender.com/api/endpoints/${schemaName}`);
            if (response.ok) {
                const data = await response.json();
                return data;
            }
...
module.exports = router;
    ...
    const saveAuthEndpoints = async (endpoints) => {
        try {
            ...
            const response = await fetch('https://apivengers.onrender.com/api/endpoints', {
                method: 'POST',
                headers: {
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`p-4 rounded-lg border-2 text-left transition-all ${selectedProvider === provider.id
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex space-x-2">
                                <button
                                    ...
                                    className={`px-4 py-2 rounded ${endpoints.length === 0
                        ...
                        {endpoints.length === 0 ? (
                            ...
                            <div className="grid gap-4">
                                {endpoints.map(endpoint => (
                                    <div key={endpoint._id} className={`bg-white border rounded-lg p-4 ${endpoint.authRequired ? 'border-l-4 border-l-green-500' : ''
                                        }`}>
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <div className="flex items-center space-x-2">
                                                    <span className={`px-2 py-1 rounded text-xs font-medium ${endpoint.method === 'GET' ? 'bg-blue-100 text-blue-800' :
                    ...
                    <div className="space-y-6">
                        <div className="bg-gray-50 p-6 rounded-lg">
                            ...
                            <button
                                ...
                                className={`px-6 py-3 rounded font-medium ${schemas.filter(s => s.name.toLowerCase().includes('user')).length === 0
                                        ? 'bg-gray-300 cursor-not-allowed'
